 <div class="col-md-6">
     <div class="card mt-5">
         <h2 class="card-header"><i class="fa-regular fa-credit-card"></i> Leaad Document Type</h2>
         <div class="card-body">
          ...
          <script type="text/javascript">
              $(function() {
                  ...
                  var table = $('.data-table').DataTable({
                      processing: true,
                      serverSide: true,
                      ajax: "{{ route('leaddocumenttype.index') }}",
                      columns: [{
                              data: 'DT_RowIndex',
                              name: 'DT_RowIndex'
                          },
                  ...
                  // Event delegation to handle click event on toggle buttons
                    ...
                    //     $.ajax({
                    //         type: 'POST',
                    //         url: "{{ route('leaddocumenttype.updateStatus') }}", // Replace 'updateStatus' with your actual route name
                    ...
                    $(document).on('change', '.status-toggle', function() {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: "{{ route('leaddocumenttype.updateStatus') }}",
                            data: { id: id, status: status },
                            success: function(response) {
                  ...
                  $('body').on('click', '.editLead', function() {
                      ...
                      $.get("{{ route('leaddocumenttype.index') }}" + '/' + product_id + '/edit', function(data) {
                          $('#modelHeading').html(
                ...
                //       $.ajax({
                //           type: 'POST',
                //           url: "{{ route('leaddocumenttype.store') }}",
                //           data: formData,
                //           contentType: false,
                //           processData: false,
                //           success: (response) => {
                ...
                //   });
                  ...
                  $('body').on('click', '.deleteLead', function() {
                      ...
                      $.ajax({
                          type: "DELETE",
                          url: "{{ route('leaddocumenttype.store') }}" + '/' + product_id,
