 <div class="col-md-6">
     <div class="card mt-5">
         <h2 class="card-header"><i class="fa-regular fa-credit-card"></i> Lead Avaliable Size</h2>
         <div class="card-body">
          ...
          <script type="text/javascript">
              $(function() {
                  ...
                  var table = $('.data-table').DataTable({
                      processing: true,
                      serverSide: true,
                      ajax: "{{ route('Leadavailablesize.index') }}",
                      columns: [{
                              data: 'DT_RowIndex',
                              name: 'DT_RowIndex'
                          },
                  ...
                  // Event delegation to handle click event on toggle buttons
                    ...
                    //     $.ajax({
                    //         type: 'POST',
                    //         url: "{{ route('Leadavailablesize.updateStatus') }}", // Replace 'updateStatus' with your actual route name
                    ...
                    $(document).on('change', '.status-toggle', function() {
                        ...
                        $.ajax({
                            type: 'POST',
                            url: "{{ route('Leadavailablesize.updateStatus') }}",
                            data: { id: id, status: status },
                            success: function(response) {
                  ...
                  $('body').on('click', '.editLead', function() {
                      ...
                      $.get("{{ route('Leadavailablesize.index') }}" + '/' + product_id + '/edit', function(data) {
                          $('#modelHeading').html(
                          "<i class='fa-regular fa-pen-to-square'></i> Edit Lead");
                          $('#saveBtn').val("edit-user");                       
                          $('#product_id').val(data.las_id);
                          $('#name').val(data.las_name);
                          $('#detail').val(data.las_status);

                           if (data.las_status === '1') {
                ...
                //       $.ajax({
                //           type: 'POST',
                //           url: "{{ route('Leadavailablesize.store') }}",
                //           data: formData,
                //           contentType: false,
                //           processData: false,
                //           success: (response) => {
                ...
                //   });
                  ...
                  $('body').on('click', '.deleteLead', function() {
                      ...
                      $.ajax({
                          type: "DELETE",
                          url: "{{ route('Leadavailablesize.store') }}" + '/' + product_id,
